...
class RichTerminal:
    ...
    def update_metrics(self, metrics: MetricsCollector):
        ...
        # Get latest metrics
        latest = metrics.get_summary()
        
        metrics_table.add_row("Total Tokens", f"{latest.get('total_tokens', 0):,}")
        metrics_table.add_row("Total Cost", f"${latest.get('total_cost', 0):.2f}")
        metrics_table.add_row("API Calls", str(latest.get('api_calls', 0)))
        metrics_table.add_row("Duration", f"{latest.get('duration', 0):.1f}s")
        metrics_table.add_row("Memory", f"{latest.get('memory_mb', 0):.1f} MB")
    ...
    def print_phase_complete(self, phase: Phase, result: Result):
